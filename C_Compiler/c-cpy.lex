...
%%
"/*"			{ comment(); return(START_COMMENT)}
"*/"			{ comment(); return(END_COMMENT)}
...
(?:\\'|"|\?|\\|a|b|f|n|r|t|v)		{ count(); return(SIMPLE_ESCAPE_SEQUENCE); }
(?:\#include\"\L+\")				{ count(); return(INCLUDE_HEADER); }
(?:\#include\[\L+\])				{ count(); return(INCLUDE_LIBRARY); }
(?:\#define\S\L+\D+)				{ count(); return(DEFINE); }

L?\"(\\.|[^\\"])*\"					{ count(); return(STRING_LITERAL); }
(\.D|D|([0-9]ES)|D\.|				){ count(); return(PREPROCESSING_NUMBER); }
...
"="			{ count(); return (DIRECT_ASSIGNMENT);}
